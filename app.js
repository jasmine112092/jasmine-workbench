...
const MAX_FILE_BYTES = 3 * 1024 * 1024; // 单个沟通文件上限 3MB（受 localStorage 总容量约 5MB 限制）
...
// 兼容新旧数据结构：返回某所大学的招生官数组 [{id,name,wechat,email}]
function getOfficers(s) {
  if (Array.isArray(s.officers) && s.officers.length) return s.officers;
  // 旧版单字段兼容
  const arr = [];
  if (s.officer || s.wechat || s.email) arr.push({ id: s.id + '_o', name: s.officer || '', wechat: s.wechat || '', email: s.email || '' });
  return arr;
}
// 每个国家分组内，按「大学名称」去重合并：
// 保留第一条为主记录，把重复院校的招生官、来访信息、文件合并进来
function dedupeVisits() {
  (STATE.visits || []).forEach(g => {
...
// 把 src 合并进 dst（保留 dst 为唯一记录）
function mergeSchool(dst, src) {
  // 合并招生官（避免重复）
  const dstOfficers = getOfficers(dst);
  const srcOfficers = getOfficers(src);
  if (dstOfficers.length || srcOfficers.length) {
    const seen = new Set();
    const merged = dstOfficers.filter(o => {
      const k = (o.name + '|' + o.wechat + '|' + o.email);
      if (seen.has(k)) return false;
      seen.add(k); return true;
    });
    srcOfficers.forEach(o => {
      const k = (o.name + '|' + o.wechat + '|' + o.email);
      if (!seen.has(k)) { seen.add(k); merged.push({ id: uid(), name: o.name, wechat: o.wechat, email: o.email }); }
    });
    dst.officers = merged;
    // 同步旧单字段（取第一个有名字的）
    const first = merged.find(o => o.name) || merged[0];
    dst.officer = first ? first.name : (dst.officer || '');
    dst.wechat = first ? first.wechat : '';
    dst.email = first ? first.email : '';
  }
  // 来访信息合并：若主记录没有而副记录有，则补上
  if (!dst.visiting && src.visiting) {
    dst.visiting = true;
    dst.visitDate = src.visitDate || '';
    dst.visitTime = src.visitTime || '';
  }
  // 文件合并
  if (Array.isArray(src.files) && src.files.length) {
    if (!Array.isArray(dst.files)) dst.files = [];
    const fids = new Set(dst.files.map(f => f.name + f.size));
    src.files.forEach(f => {
      const k = f.name + f.size;
      if (!fids.has(k)) { fids.add(k); dst.files.push(f); }
    });
  }
}
...
function viewHome() {
  ...
  const allVS = allVisitSchools();
  const admitted = admittedSchools();
  const submitted = afterSchools();

  // ===== KPI 概览条 =====
  const openTodos = (STATE.todos || []).filter(x => !x.done).length;
  const kpis = [
    { n: todayTasks.length + incTasks.length, t: '今日待办', c: 'blue', go: 'completion' },
    { n: visits.length, t: '今日来访', c: 'pink', go: 'visits' },
    { n: allVS.length, t: '来访大学', c: 'yellow', go: 'visits' },
    { n: allC.length, t: '申请院校', c: 'green', go: 'students' },
    { n: submitted.length, t: '已递交', c: 'purple', go: 'after' },
    { n: admitted.length, t: '已录取', c: 'red', go: 'admission' },
  ].map(k => `<button class="kpi ${k.c}" data-act="nav" data-route="${k.go}">
      <div class="n">${k.n}</div><div class="t">${k.t}</div>
    </button>`).join('');

  // ===== 申请进度分布（0-49 / 50-79 / 80-99 / 100）=====
  const buckets = [
    { label: '0-49%', color: 'var(--red)', test: p => p < 50 },
    { label: '50-79%', color: 'var(--yellow)', test: p => p >= 50 && p < 80 },
    { label: '80-99%', color: 'var(--blue-500)', test: p => p >= 80 && p < 100 },
    { label: '100%', color: 'var(--green)', test: p => p >= 100 },
  ];
  const bmax = Math.max(1, ...buckets.map(b => allC.filter(c => b.test(c.progress || 0)).length));
  const distBars = buckets.map(b => {
    const n = allC.filter(c => b.test(c.progress || 0)).length;
    const w = Math.round(n / bmax * 100);
    return `<div class="vg-row">
      <div class="vg-label">${b.label}</div>
      <div class="vg-bar"><i style="width:${w}%;background:${b.color}"></i></div>
      <div class="vg-n">${n}</div>
    </div>`;
  }).join('');
  ...
  const countryMap = {};
  STATE.students.forEach(s => (s.countries || []).forEach(c => {
    const nm = c.country || '未分类';
    countryMap[nm] = (countryMap[nm] || 0) + (c.schools ? c.schools.length : 0);
  }));
  const cntArr = Object.keys(countryMap).sort((a, b) => countryMap[b] - countryMap[a]);
  const cntMax = Math.max(1, ...cntArr.map(c => countryMap[c]));
  const countryBars = cntArr.slice(0, 6).map(c => {
    const w = Math.round(countryMap[c] / cntMax * 100);
    return `<div class="vg-row">
      <div class="vg-label">${escapeHtml(c)}</div>
      <div class="vg-bar"><i style="width:${w}%"></i></div>
      <div class="vg-n">${countryMap[c]}</div>
    </div>`;
  }).join('') || '<div class="empty">暂无学生数据</div>';

  // ===== 大学来访月份分布（未来12个月内）=====
  const visitMonths = {};
  allVS.filter(s => s.visiting && s.visitDate).forEach(s => {
    const m = (s.visitDate || '').slice(0, 7);
    if (m) visitMonths[m] = (visitMonths[m] || 0) + 1;
  });
  const vmArr = Object.keys(visitMonths).sort();
  const vmMax = Math.max(1, ...vmArr.map(m => visitMonths[m]));
  const visitMonthBars = vmArr.slice(-8).map(m => {
    const w = Math.round(visitMonths[m] / vmMax * 100);
    const [yy, mm] = m.split('-');
    return `<div class="vg-row">
      <div class="vg-label">${yy}年${Number(mm)}月</div>
      <div class="vg-bar"><i style="width:${w}%;background:var(--pink-400)"></i></div>
      <div class="vg-n">${visitMonths[m]}</div>
    </div>`;
  }).join('') || '<div class="empty">暂无来访计划</div>';

  // ===== 录取情况 =====
  const admittedN = admitted.length;
  const pendingA = Math.max(0, submitted.length - admittedN);
  const admitTotal = submitted.length;
  const admitMax = Math.max(1, admitTotal);
  const admitW = Math.round(admittedN / admitMax * 100);
  ...
  <div class="card">
    <div class="card-title"><span class="dot" style="background:var(--purple-500)"></span>数据可视化 · 申请与录取总览</div>
    <div class="vg-grid">
      <div class="vg-col">
        <div class="vg-title">申请进度分布</div>
        ${distBars}
      </div>
      <div class="vg-col">
        <div class="vg-title">按国家申请院校数</div>
        ${countryBars}
      </div>
      <div class="vg-col">
        <div class="vg-title">大学来访·月份分布</div>
        ${visitMonthBars}
      </div>
      <div class="vg-col">
        <div class="vg-title">录取情况</div>
        <div class="vg-row">
          <div class="vg-label">已录取</div>
          <div class="vg-bar"><i style="width:${admitW}%;background:var(--green)"></i></div>
          <div class="vg-n">${admittedN}</div>
        </div>
        <div class="vg-row">
          <div class="vg-label">待结果</div>
          <div class="vg-bar"><i style="width:${admitTotal ? Math.round(pendingA / admitTotal * 100) : 0}%;background:var(--yellow)"></i></div>
          <div class="vg-n">${pendingA}</div>
        </div>
        <div class="section-label" style="margin-top:10px">共递交 ${admitTotal} 所院校</div>
        ${admitTotal ? `<div class="vg-pct"><i style="width:${admitW}%;background:linear-gradient(90deg,var(--green),var(--blue-400))"></i><span>录取率 ${admitTotal ? Math.round(admittedN / admitTotal * 100) : 0}%</span></div>` : ''}
      </div>
    </div>
  </div>`;
}
...
function visitSchoolRow(s, g) {
  ...
  // 招生官列表（支持同一所大学多位招生官）
  const officers = getOfficers(s);
  const officerHtml = officers.map((o, oi) => `
    <span class="v-item v-officer">
      👤 ${escapeHtml(o.name || '招生官')}
      ${o.wechat ? `<span class="v-o-wechat">微信：${escapeHtml(o.wechat)}</span>` : ''}
      ${o.email ? `<span class="v-o-email">邮箱：${escapeHtml(o.email)}</span>` : ''}
      <button class="mini-btn v-o-del" data-act="del-visit-officer" data-cid="${g.id}" data-sid="${s.id}" data-oi="${oi}" title="移除该招生官">✕</button>
    </span>`).join('');
  ...
  return `<div class="visit-school ${s.visiting ? 'visiting' : ''}">
    <div class="visit-main">
      ...
      <div class="visit-sub">
        ${officerHtml || '<span class="v-item v-officer-empty">暂无招生官信息</span>'}
        <span class="v-add-officer">
          <input class="input v-officer-input" placeholder="＋招生官" data-officer-name="${s.id}" maxlength="20"/>
          <input class="input v-officer-wx" placeholder="微信" data-officer-wx="${s.id}" maxlength="30"/>
          <button class="mini-btn v-o-add" data-act="add-visit-officer" data-cid="${g.id}" data-sid="${s.id}" title="添加招生官">＋</button>
        </span>
    ...
    <div class="visit-files">
      <div class="vf-head">
        ...
        <p class="vf-hint">支持 PDF / Word / 图片 / Excel / PPT 等，单个不超过 3MB，保存在本机，可随时下载查看。</p>
      </div>
...
function viewVisitsDashboard() {
  const visiting = allVisitSchools().filter(s => s.visiting && s.visitDate);
  if (!visiting.length) {
    return `<div class="empty">还没有本学期来访记录，勾选「本学期来访」并填写日期后，这里会生成汇总。</div>`;
  }
  ...
  (STATE.visits || []).forEach(g => {
    (g.schools || []).forEach(s => {
      if (s.visiting && s.visitDate) {
        if (!countryMap[g.country]) countryMap[g.country] = [];
        countryMap[g.country].push(s);
      }
    });
  });
  const countries = Object.keys(countryMap).sort((a, b) => countryMap[b].length - countryMap[a].length);
  const countryNum = countries.length;
  const maxCountry = Math.max(1, ...countries.map(c => countryMap[c].length));

  // 按月份统计
  const monthMap = {};
  visiting.forEach(s => {
    const m = (s.visitDate || '').slice(0, 7); // YYYY-MM
  ...
  // 国家条形图
  const countryBars = countries.map(c => {
    const n = countryMap[c].length;
    const w = Math.round(n / maxCountry * 100);
    const schools = countryMap[c].map(s => escapeHtml(s.name)).join('、');
    return `<div class="vd-row">
      <div class="vd-label" title="${schools}">${escapeHtml(c)}<span class="vd-n">${n}</span></div>
      <div class="vd-bar"><i style="width:${w}%"></i></div>
    </div>`;
  }).join('');
  ...
  const monthBars = months.map(m => {
    const n = monthMap[m];
    const w = Math.round(n / maxMonth * 100);
    const ms = visiting.filter(s => (s.visitDate || '').slice(0, 7) === m)
      .map(s => escapeHtml(s.name)).join('、');
    const [yy, mm] = m.split('-');
    return `<div class="vd-row">
      <div class="vd-label" title="${ms}">${yy}年${Number(mm)}月<span class="vd-n">${n}</span></div>
      <div class="vd-bar"><i style="width:${w}%"></i></div>
    </div>`;
  }).join('');
  ...
  <div class="vd-dash">
    <div class="vd-overview">
      <div class="vd-ov"><div class="n">${visiting.length}</div><div class="t">本学期来访大学</div></div>
      <div class="vd-ov"><div class="n">${countryNum}</div><div class="t">涉及国家</div></div>
      <div class="vd-ov"><div class="n">${months.length}</div><div class="t">覆盖月份</div></div>
    </div>
    <div class="vd-cols">
      <div class="vd-col">
        <div class="vd-col-title">按国家统计来访次数</div>
        ${countryBars || '<div class="empty">暂无</div>'}
      </div>
      <div class="vd-col">
        <div class="vd-col-title">按月份统计来访次数</div>
        ${monthBars || '<div class="empty">暂无</div>'}
      </div>
    </div>
  </div>`;
}
...
function viewVisits() {
  ...
  // 根据搜索关键词过滤院校（同时匹配大学名、多招生官、微信、邮箱）
  const filterSchools = (schools) => {
    if (!q) return schools;
    return (schools || []).filter(s => {
      const officers = getOfficers(s);
      const searchable = [s.name]
        .concat(officers.map(o => [o.name, o.wechat, o.email]));
      return searchable.flat().some(v =>
  ...
  <div class="card">
    ...
    <p class="hint">共 ${total} 所院校（重复大学已自动合并）、${visiting} 所本学期来访。每所大学可添加多位招生官，勾选「本学期来访」并填写日期后会自动标记在首页月历上。</p>
    <div class="visit-toolbar">
      ...
      <button class="btn ${visitDashOpen ? 'pink' : 'ghost'}" data-act="toggle-visit-dash" style="white-space:nowrap">📊 来访汇总看板</button>
    ...
    ${visitDashOpen ? viewVisitsDashboard() : ''}
    ${manualForm}
...
document.addEventListener('click', e => {
        ...
        return {
          ...
          schools: (p.schools || []).map(ps => {
            ...
            return {
              ...
              officers: old && Array.isArray(old.officers) ? old.officers : [],
  ...
  if (act === 'add-visit-manual') {
    ...
    const officers = officer || contact ? [{ id: uid(), name: officer, wechat: '', email: contact }] : [];
    const school = {
  ...
  if (act === 'toggle-visit-dash') {
    visitDashOpen = !visitDashOpen;
    render(); return;
  }
  ...
  if (act === 'add-visit-officer') {
    const g = (STATE.visits || []).find(x => x.id === el.dataset.cid); if (!g) return;
    const s = g.schools.find(x => x.id === el.dataset.sid); if (!s) return;
    const name = (document.querySelector('[data-officer-name="' + s.id + '"]') || {}).value || '';
    const wx = (document.querySelector('[data-officer-wx="' + s.id + '"]') || {}).value || '';
    if (!name.trim() && !wx.trim()) return;
    if (!Array.isArray(s.officers)) {
      s.officers = getOfficers(s);
      if (s.officer || s.wechat || s.email) {
        // getOfficers 已包含旧字段，标记
      }
    }
    s.officers.push({ id: uid(), name: name.trim(), wechat: wx.trim(), email: '' });
    // 同步旧字段
    if (!s.officer) s.officer = name.trim();
    if (!s.wechat) s.wechat = wx.trim();
    save(); render(); return;
  }
  if (act === 'del-visit-officer') {
    const g = (STATE.visits || []).find(x => x.id === el.dataset.cid); if (!g) return;
    const s = g.schools.find(x => x.id === el.dataset.sid); if (!s) return;
    const officers = getOfficers(s);
    const oi = Number(el.dataset.oi);
    if (!officers[oi]) return;
    officers.splice(oi, 1);
    if (officers.length) {
      s.officers = officers;
      const first = officers.find(o => o.name) || officers[0];
      s.officer = first ? first.name : '';
      s.wechat = first ? first.wechat : '';
      s.email = first ? first.email : '';
    } else {
      delete s.officers;
      s.officer = ''; s.wechat = ''; s.email = '';
    }
    save(); render(); return;
  }
...
document.addEventListener('change', e => {
  ...
  if (e.target && e.target.dataset.act === 'upload-visit-file') {
    ...
    function finishUpload(big, fail, any) {
      ...
      else if (big) toast('⚠️ ' + big + ' 个文件超过 3MB 限制未上传');
